...
export class RestApiService {
  ...
  updatePersonal(id:number, Personal:Personal): Observable<Personal> {
    return this.http.put<Personal>(this.apiURL + 'personal/' + id, JSON.stringify(Personal), this.httpOptions)
    .pipe(
      retry(1),
      catchError(this.handleError)
    )
  ...
  eliminarPersonal(id:number){
    return this.http.delete<Personal>(this.apiURL + 'personal/' + id, this.httpOptions)
    .pipe(
      retry(1),
      catchError(this.handleError)
    )
